...
MainWindow::MainWindow(QWidget *parent)
  ...
  for (int i = 0; i < 29; i++) {
    ...
    connect(numButtons[i], &QPushButton::released, this,
            &MainWindow::ButPressed);
  }

  QPushButton *clearButton;
  clearButton = findChild<QPushButton *>("ClearButton");
  connect(clearButton, &QPushButton::released, this,
          &MainWindow::ClearLineEdit);

  QPushButton *equilButton;
  equilButton = findChild<QPushButton *>("Equals");
  connect(equilButton, &QPushButton::released, this,
          &MainWindow::EqualButton_clicked);

  QPushButton *graphicButton;
  graphicButton = findChild<QPushButton *>("Graphic");
  connect(graphicButton, &QPushButton::released, this,
          &MainWindow::GraphicButton_clicked);
}
...
void MainWindow::EqualButton_clicked() {
  ...
  if (expression.contains("x")) {
    QDoubleSpinBox *xValueSpinBox =
        findChild<QDoubleSpinBox *>("xExpressionValue");
...
void MainWindow::GraphicButton_clicked() {
  ...
  if (expression.contains("x")) {
    ...
    while (xBeginValue < xEndValue) {
      ...
      long double y_tmp_value = calculating((tmp_str));
